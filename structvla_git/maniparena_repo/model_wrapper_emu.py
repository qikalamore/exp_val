...
import sys
sys.path.append("/remote-home/linweihao/structvla/reference/Emu3")
...
class EmuVLAModel:
    ...
    def init_config(self, device):
        ...
        # fast tokenization
        fast_path = "/remote-home/linweihao/structvla/pretrain/fast"
    ...
    def _get_image_by_keys(self, image, *keys):
        for key in keys:
            value = image.get(key)
    ...
    def _encode_image(self, image):
        ...
        image_x = self.image_processor(image, return_tensors="pt")["pixel_values"].to(self.device)
        return self.image_tokenizer.encode(image_x)
    ...
    def preprocess(self, image):
        # preprocess bimanual views: front camera + left/right wrist or side views
        agent_view = self._get_image_by_keys(image, "front", "full_image", "camera_front")
        left_view = self._get_image_by_keys(
            image,
            "left",
            "left_wrist_image",
            "left_image",
        ...
        right_view = self._get_image_by_keys(
            image,
            "right",
            "right_wrist_image",
            "right_image",
        ...
        if agent_view is None:
            raise KeyError("Missing front view. Expected one of: front/full_image/camera_front")
        if left_view is None or right_view is None:
            raise KeyError(
                "Missing bimanual wrist views. Expected left/right views such as "
                "left,right or left_wrist_image,right_wrist_image."
            )

        image_code = self._encode_image(agent_view)
        left_gripper_code = self._encode_image(left_view)
        right_gripper_code = self._encode_image(right_view)
    ...
    def step(self, image, goal):
        ...
        image_code, left_gripper_code, right_gripper_code = self.preprocess(image)

        prompt,neg_prompt = goal,""

        video_code = image_code.unsqueeze(1)
        gripper_code = (
            left_gripper_code.unsqueeze(1),
            right_gripper_code.unsqueeze(1),
        ) if self.use_gripper else None
    ...
    def unormalize_action(self, action):
        ...
        if action.shape[-1] != self.action_dim:
            raise ValueError(f"Expected last action dim {self.action_dim}, got {action.shape[-1]}")
