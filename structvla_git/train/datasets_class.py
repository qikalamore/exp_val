...
class Emu3PlannerDataset(Emu3SFTDataset):
    ...
    def _scene_primary_images(self, scene: dict):
        return scene["image"]

    def _scene_aux_image_lists(self, scene: dict, primary_len: int):
        ...
        if use_grip and isinstance(grip_paths, (list, tuple)) and len(grip_paths) == primary_len:
            return [grip_paths]
        return []

    def _scene_key(self, scene: dict, key_from: str, key_field: Optional[str], scene_idx: int):
        if key_from == "index":
        ...
        if key_from == "stem":
            try:
                img_list = self._scene_primary_images(scene)
            except KeyError:
                img_list = None
            if not img_list:
                return None
    ...
    def __init__(self, args: "DataArguments", tokenizer: "Emu3Tokenizer"):
        ...
        for si, scene in enumerate(self.data):
            N = len(self._scene_primary_images(scene))
    ...
    def __getitem__(self, index: int):
        ...
        img_paths = self._scene_primary_images(scene)
        N = len(img_paths)
        aux_paths_list = self._scene_aux_image_lists(scene, N)
        ...
        if getattr(self, "raw_image", False) and getattr(self, "batch_encode", True) and len(all_idx) > 0:
            ...
            aux_tokens_list = [self._load_frames_tokens_batch(paths, all_idx) for paths in aux_paths_list]
        else:
            frames_tokens = [self._load_single_frame_tokens(img_paths, ii) for ii in all_idx]
            aux_tokens_list = [
                [self._load_single_frame_tokens(paths, ii) for ii in all_idx]
                for paths in aux_paths_list
            ]
        ...
        for t in range(len(all_idx) - 1):
            ...
            for aux_tokens in aux_tokens_list:
                p += self.format_video_prompt(aux_tokens[t])
            seg_ctx_prompts.append(p)
        seg_tgt_prompt = self.format_video_prompt(frames_tokens[-1])
        for aux_tokens in aux_tokens_list:
            seg_tgt_prompt += self.format_video_prompt(aux_tokens[-1])
...
class Emu3PlannerRealRobotDataset(Emu3PlannerDataset):
    """
    Planner variant for dual-arm data stored as cam_high/cam_left_wrist/cam_right_wrist.
    It reuses Emu3PlannerDataset and only adapts how image lists are fetched.
    """

    def _scene_primary_images(self, scene: dict):
        return scene["cam_high"]

    def _scene_aux_image_lists(self, scene: dict, primary_len: int):
        if not bool(getattr(self, "use_gripper", False)):
            return []

        aux_lists = []
        for key in ("cam_left_wrist", "cam_right_wrist"):
            paths = scene.get(key, None)
            if isinstance(paths, (list, tuple)) and len(paths) == primary_len:
                aux_lists.append(paths)
        return aux_lists
