...
DEFAULT_SIM_PKL = "/remote-home/linweihao/maniparena_dataset_real/processed_data/meta/real_all_dualarm_norm.pkl"
DEFAULT_SIM_OUT = "/remote-home/linweihao/maniparena_dataset_real/processed_data/structout_testset"
DUAL_ARM_DIM = 14
KESTEP_PRIORITY = {"flip": 0, "still": 1, "backfill": 2}
SIM_GRIPPER_NORM_MIN = -1.0
SIM_GRIPPER_NORM_MAX = 1.0
SIM_GRIPPER_ABS_MIN = 0.0
SIM_GRIPPER_ABS_MAX = 4.5
SIM_GRIPPER_OPEN_THRESHOLD = 3.3
SIM_GRIPPER_CLOSE_THRESHOLD = 1.0
SIM_GRIPPER_BINARIZE_MODE = "ge3p3_le1p0_hold"
...
def action_motion_view(actions: np.ndarray) -> np.ndarray:
    """Convert pose-like sim actions to per-frame delta/velocity actions."""
    acts = np.asarray(actions, dtype=np.float32)
    if acts.ndim != 2:
        return acts
    motion = np.zeros_like(acts, dtype=np.float32)
    if acts.shape[0] > 1:
        motion[1:, :6] = acts[1:, :6] - acts[:-1, :6]
    if acts.shape[1] > 6:
        motion[:, 6:] = acts[:, 6:]
    return motion
...
def action_inst_mags(actions: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """Instantaneous motion magnitude after converting sim pose actions to deltas."""
    if actions.ndim != 2 or actions.shape[1] < 6:
        T = actions.shape[0]
        return np.full(T, 1e9, dtype=np.float32), np.full(T, 1e9, dtype=np.float32)
    motion = action_motion_view(actions)
    pos_mag = np.sqrt(np.sum(np.square(motion[:, :3].astype(np.float32)), axis=1))
    rot_mag = np.sqrt(np.sum(np.square(motion[:, 3:6].astype(np.float32)), axis=1))
...
def window_action_magnitude(actions: np.ndarray) -> float:
    ...
    if actions.ndim != 2 or actions.shape[1] < 6 or actions.shape[0] == 0:
        return 999.0
    motion = action_motion_view(actions)
    mean6 = np.mean(motion[:, :6], axis=0)
    return float(np.sum(np.abs(mean6)) * actions.shape[0])

def binarize_with_mode(x: float, mode: str) -> int:
    if mode == SIM_GRIPPER_BINARIZE_MODE:
        return int(x >= SIM_GRIPPER_OPEN_THRESHOLD)
    if mode == "gt05":
        return int(x > 0.5)
    elif mode == "gt0":
        return int(x > 0.0)
...
def binarize_gripper_sequence(
    g_values: np.ndarray,
    open_threshold: float = SIM_GRIPPER_OPEN_THRESHOLD,
    close_threshold: float = SIM_GRIPPER_CLOSE_THRESHOLD,
) -> np.ndarray:
    """Map continuous gripper values to binary states with a hold region.

    Policy:
      - val >= open_threshold  -> open (1)
      - val <= close_threshold -> close (0)
      - close_threshold < val < open_threshold -> hold previous state

    If the sequence starts in the hold region, initialize from the first
    explicit open/close command that appears later; if none appears, default
    to close.
    """
    vals = np.asarray(g_values, dtype=np.float32).reshape(-1)
    if vals.size == 0:
    ...
    for v in vals:
        if v >= open_threshold:
            state = 1
            break
        if v <= close_threshold:
            state = 0
            break

    bits = np.empty(vals.shape[0], dtype=np.int32)
...
def get_main_image_field(ep: Dict[str, Any], T: int):
    return ep.get("image", ep.get("cam_high", [""] * T))


def get_gripper_image_field(ep: Dict[str, Any], fallback: Any):
    return ep.get("image_gripper", ep.get("gripper_image", ep.get("cam_left_wrist", fallback)))


def is_dual_arm(actions: np.ndarray) -> bool:
    return actions.ndim == 2 and actions.shape[1] >= DUAL_ARM_DIM


def denormalize_gripper_values(
    g_values: np.ndarray,
    norm_min: float = SIM_GRIPPER_NORM_MIN,
    norm_max: float = SIM_GRIPPER_NORM_MAX,
    abs_min: float = SIM_GRIPPER_ABS_MIN,
    abs_max: float = SIM_GRIPPER_ABS_MAX,
) -> np.ndarray:
    """Linearly map normalized gripper values from [-1, 1] back to [0, 4.5]."""
    vals = np.asarray(g_values, dtype=np.float32)
    vals = np.clip(vals, norm_min, norm_max)
    scale = (abs_max - abs_min) / float(norm_max - norm_min)
    return (vals - norm_min) * scale + abs_min


def get_arm_action_views(actions: np.ndarray) -> List[Tuple[str, np.ndarray, np.ndarray]]:
    if is_dual_arm(actions):
        return [
            ("left", actions[:, :7], denormalize_gripper_values(actions[:, 6])),
            ("right", actions[:, 7:14], denormalize_gripper_values(actions[:, 13])),
        ]
    return [("single", actions, denormalize_gripper_values(actions[:, -1]))]


def attach_event_arm(items: List[Dict[str, Any]], arm_name: str) -> List[Dict[str, Any]]:
    for item in items:
        item["event_arm"] = arm_name
    return items


def merge_keysteps_by_step(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    merged: Dict[int, Dict[str, Any]] = {}
    for item in sorted(
        items,
        key=lambda x: (
            int(x["step_keystep"]),
            KESTEP_PRIORITY.get(str(x.get("type", "")), 99),
            int(x.get("lookahead_used", 0)),
        ),
    ):
        step = int(item["step_keystep"])
        if step not in merged:
            merged[step] = dict(item)
            continue
        prev = merged[step]
        prev_arm = str(prev.get("event_arm", "single"))
        cur_arm = str(item.get("event_arm", "single"))
        if prev_arm != cur_arm:
            prev["event_arm"] = "both"
    return sorted(merged.values(), key=lambda x: int(x["step_keystep"]))
...
def pick_keystep_after_grip_settle_libero(frames: List[Dict[str, Any]],
                                   ...
                                   grip_settle_win: int,
                                   pre_move_horizon: int,
                                   actions_view: Optional[np.ndarray] = None) -> Tuple[int, int, float, float, bool]:
    """
    ...
    settle_end = min(settle_end, end_t)
...
def select_most_stable_in_window(steps: List[int], actions: np.ndarray, g_values: np.ndarray,
    ...
    if mode_used == SIM_GRIPPER_BINARIZE_MODE:
        grip_bits = binarize_gripper_sequence(g_values).tolist()
    else:
...
def make_backfill_item(frames: List[Dict[str, Any]],
    ...
    a_prev = np.asarray(actions[rel_b], dtype=np.float32)
    a_curr = np.asarray(actions[rel], dtype=np.float32)
    dp = l2(a_curr[:3] - a_prev[:3])
    dr = l2(a_curr[3:6] - a_prev[3:6])

    gb = int(grip_bits[rel_b])
    ga = int(grip_bits[rel])
...
def flatten_pkl_bridge(pkl_path: str, test: bool = False) -> List[Dict[str, Any]]:
    ...
    for epi, ep in enumerate(episodes):
        ...
        assert actions.ndim == 2 and actions.shape[1] >= 7, "action must have shape [T,>=7]"
        T = actions.shape[0]
        imgs = get_main_image_field(ep, T)
...
def pick_keystep_after_grip_settle_bridge(frames: List[Dict[str, Any]],
    ...
    while t <= end_t:
        if grip_bits[t] != new_state:
            break
        horizon_end = min(t + int(pre_move_horizon), end_t, T - 1)
        ...
        if first_big_k is not None:
            ks_t = min(max(settle_end, first_big_k - 1), end_t)
            ks_idx = int(indices[ks_t])
...
BASE_DEFAULTS = {
    'dataset': DEFAULT_SIM_PKL,
    'out_dir': DEFAULT_SIM_OUT,
    'min_changes': 0,
    'max_lookahead': 12,
...
REAL_DUALARM_NORM_OVERRIDES = {
    # This dataset contains long exact-static spans in normalized action space.
    # Use stricter still-frame spacing/guards so we do not over-sample those regions.
    'min_changes': 0,
    'max_lookahead': 12,
    'min_offset_after_flip': 1,
    'pre_move_horizon': 2,
...
DATASET_PRESETS = {
    ...
    'real': {
        ...
        'dataset': '',
        'out_dir': '',
        'overrides': dict(REAL_DUALARM_NORM_OVERRIDES),
    },
    'real_dualarm': {
        'logic_profile': 'libero',
        'dataset': '',
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument('--datasets', type=str, default='real',
                    help='Dataset name. simpler/libero/real/real_dualarm inject validated defaults; any other name uses generic defaults.')
...
def run_bridge_profile(args):
    ...
    for ep_id, indices in tqdm(grouped.items(), desc='Episodes'):
        ...
        for arm_name, arm_actions, g_values in get_arm_action_views(actions):
            mode_used = SIM_GRIPPER_BINARIZE_MODE
            grip_bits = binarize_gripper_sequence(g_values).tolist()
            cps = find_change_points(grip_bits) if len(grip_bits) >= 2 else []
            if len(cps) < args.min_changes:
                cps = []
            th = compute_episode_thresholds(arm_actions, args.flip_pos_pctl, args.flip_rot_pctl, args.still_pos_pctl, args.still_rot_pctl, args.flip_scale, args.still_scale, args.flip_pos_min, args.flip_rot_min, args.still_pos_max, args.still_rot_max)
            ...
            for cp in cps:
                ...
                if window_len <= max(args.min_window, 1):
                    ...
                    if window_action_magnitude(win_actions) < args.mag_thresh:
                        continue
                arm_keysteps.append({'type':'flip','cp':int(cp),'idx_before':int(indices[before_rel]),'idx_keystep':int(ks_idx_global),'step_before':int(step_before),'step_keystep':int(step_keystep),'gripper_before':int(grip_bits[cp-1]),'gripper_after':int(grip_bits[cp]),'pos_delta_keystep':float(pos_at_ks),'rot_delta_keystep':float(rot_at_ks),'lookahead_used':int(lookahead_used),'binarize_mode':mode_used,'found_premove':bool(found_premove),'event_arm':arm_name,'flip_enter_pos':float(th['flip_enter_pos']),'flip_enter_rot':float(th['flip_enter_rot']),'still_pos_thr':float(th['still_pos']),'still_rot_thr':float(th['still_rot'])})
            if not args.no_still_keys:
                stills = collect_still_keysteps_bridge(frames=frames, indices=indices, steps=steps, actions=arm_actions, grip_bits=grip_bits, mode_used=mode_used, pos_mag_thresh=th['still_pos'], rot_mag_thresh=th['still_rot'], win=max(1, int(args.still_win)), min_gap=max(1, int(args.still_min_gap)), existing_steps=[k['step_keystep'] for k in arm_keysteps], flip_steps=[k['step_keystep'] for k in arm_keysteps], flip_guard=int(args.still_flip_guard), max_scale=float(args.still_max_scale), ema_win=int(args.still_ema_win), hysteresis=float(args.still_hysteresis), ratio_req=float(args.still_ratio))
                for item in stills:
                ...
                arm_keysteps.extend(attach_event_arm(stills, arm_name))
            ...
            def append_if_valid(prefer_step: int, forbid: List[int], collected: List[Dict[str, Any]]):
                ...
                if cand is not None:
                    item = make_backfill_item(frames, indices, steps, arm_actions, grip_bits, mode_used, cand)
                    item["event_arm"] = arm_name
                    item['flip_enter_pos'] = float(th['flip_enter_pos'])
                    item['flip_enter_rot'] = float(th['flip_enter_rot'])
                    item['still_pos_thr'] = float(th['still_pos'])
        ...
        all_keysteps = merge_keysteps_by_step(all_candidates)
